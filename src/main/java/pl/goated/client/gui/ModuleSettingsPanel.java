...
public class ModuleSettingsPanel {
		...
		try {
			...
			for (Setting<?> setting : settings) {
				if (setting instanceof ColorSetting colorSetting) {
					...
					components.add(new FloatSettingComponent(floatSetting, PADDING, yOffset, width - PADDING * 2));
					yOffset += SETTING_HEIGHT + 10;
				}
			}
		} catch (Exception e) {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (activeColorPicker != null) {
			if (activeColorPicker.mouseClicked(mouseX, mouseY, button)) return true;
			else { activeColorPicker = null; return false; }
		}
		...
		for (SettingComponent component : components) {
			SettingComponent.ClickResult result = component.mouseClicked(x, y, mouseX, mouseY, button);
			if (result == SettingComponent.ClickResult.OPEN_COLOR_PICKER && component instanceof ColorSettingComponent colorComp) {
				activeColorPicker = new ColorPicker(colorComp.setting, (int) mouseX, (int) mouseY);
				return true;
			} else if (result == SettingComponent.ClickResult.HANDLED) {
				return true;
			}
		}
		return false;
	...
	public void mouseReleased(double mouseX, double mouseY, int button) {
		...
		for (SettingComponent component : components) {
			component.mouseReleased(x, y, mouseX, mouseY);
		}
	}
	...
	private interface SettingComponent {
		enum ClickResult { NONE, HANDLED, OPEN_COLOR_PICKER }
		void render(DrawContext context, int panelX, int panelY, int mouseX, int mouseY, GuiModule guiModule);
		ClickResult mouseClicked(int panelX, int panelY, double mouseX, double mouseY, int button);
		default void mouseReleased(int panelX, int panelY, double mouseX, double mouseY) {}
	}
	
	private static class FloatSettingComponent implements SettingComponent {
		private final FloatSetting setting;
		private final int x, y, width;
		private boolean dragging = false;
		
		public FloatSettingComponent(FloatSetting setting, int x, int y, int width) {
		...
		public void render(DrawContext context, int panelX, int panelY, int mouseX, int mouseY, GuiModule guiModule) {
			...
			context.drawText(MinecraftClient.getInstance().textRenderer,
				setting.getDisplayValue(), absoluteX + width - 50, absoluteY + 5,
				guiModule.textColor.getValue(), false);
			
			// Pasek suwaka
			int sliderY = absoluteY + 20;
			int sliderHeight = 10;
			
			// Tło suwaka
			RenderUtil.drawRoundedRect(context, absoluteX, sliderY, width, sliderHeight, 5,
				RenderUtil.adjustAlpha(guiModule.borderColor.getValue(), 80));
			
			// Wypełnienie suwaka
			int fillWidth = (int) (width * setting.getPercent());
			if (fillWidth > 0) {
				RenderUtil.drawRoundedRect(context, absoluteX, sliderY, fillWidth, sliderHeight, 5, 0xFF00CC00);
			}
			
			// Kursor suwaka
			int cursorX = absoluteX + fillWidth - 5;
			RenderUtil.drawRoundedRect(context, cursorX, sliderY - 3, 10, sliderHeight + 6, 5, 0xFFFFFFFF);
			
			// Aktualizacja gdy przeciągamy
			if (dragging) {
				float percent = (float)(mouseX - absoluteX) / width;
				setting.setFromPercent(percent);
				GoatedClient.getInstance().getConfigManager().save();
			}
		}
		
		@Override
		public ClickResult mouseClicked(int panelX, int panelY, double mouseX, double mouseY, int button) {
			int absoluteX = panelX + x;
			int absoluteY = panelY + y;
			int sliderY = absoluteY + 20;
			
			if (mouseX >= absoluteX && mouseX <= absoluteX + width &&
				mouseY >= sliderY - 3 && mouseY <= sliderY + 13) {
				dragging = true;
				float percent = (float)(mouseX - absoluteX) / width;
				setting.setFromPercent(percent);
		...
		@Override
		public void mouseReleased(int panelX, int panelY, double mouseX, double mouseY) {
			if (dragging) {
				dragging = false;
				GoatedClient.getInstance().getConfigManager().save();
			}
	...
	private static class ColorSettingComponent implements SettingComponent {
		...
		public ClickResult mouseClicked(int panelX, int panelY, double mouseX, double mouseY, int button) {
			...
			if (mouseX >= previewX && mouseX <= previewX + 70 &&
				...
				return ClickResult.OPEN_COLOR_PICKER;
			}
			return ClickResult.NONE;
		}
	}
